(function () {
  ...
  function renderNode(value, path = [], key = "") {
    if (Array.isArray(value)) {
      return `<section class="space-y-3" data-array="${path.join(".")}">
        <div class="flex items-center justify-between"><h3 class="font-display text-lg font-bold">${title(key)}</h3>
        <button type="button" data-add="${path.join(".")}" class="${classes.button}"><i class="bi bi-plus-lg"></i> Add</button></div>
    ...
    if (value && typeof value === "object") {
      return `<fieldset class="space-y-3"><legend class="${path.length > 1 ? "mb-2 font-bold" : "sr-only"}">${title(key)}</legend>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">${Object.entries(value).map(([childKey, child]) => {
  ...
  function buildUI() {
    ...
    document.getElementById("editor-fields").addEventListener("click", editArray);
  ...
  function open() {
    ...
    document.getElementById("editor-fields").innerHTML = renderNode(draft);
