...
const notesCode = fs.readFileSync("char/js/tracker/notes.js", "utf8")
  ...
  .replace(/export /g, "");
const stateCode = fs.readFileSync("char/js/tracker/state.js", "utf8")
  .replace(/^import[\s\S]*?;\r?\n/gm, "")
  .replace(/export /g, "");
const appCode = fs.readFileSync("char/js/tracker/index.js", "utf8")
  .replace(/^import[\s\S]*?;\r?\n/gm, "")
...
const karmaSource = `window.character = ${fs.readFileSync("char/karma/character.json", "utf8")};`;
...
const allySource = `window.character = ${fs.readFileSync("char/ally/character.json", "utf8")};`;
