...
function renderSettings() {
  ...
  document.getElementById("campaign-character-styles").innerHTML = characters.map((character) => `<label class="flex items-center justify-between gap-3"><span class="truncate">${escapeHTML(character.document?.name || character.id)}</span><select data-style="${escapeAttribute(character.id)}" class="rounded-lg border border-stone-300 bg-white px-3 py-2 dark:border-white/15 dark:bg-stone-900"><option value="">Campaign default</option><option value="v1"${settings.characterSheetStyleOverrides[character.id] === "v1" ? " selected" : ""}>Style v1</option><option value="v2"${settings.characterSheetStyleOverrides[character.id] === "v2" ? " selected" : ""}>Style v2</option><option value="v3"${settings.characterSheetStyleOverrides[character.id] === "v3" ? " selected" : ""}>Style v3</option><option value="v4"${settings.characterSheetStyleOverrides[character.id] === "v4" ? " selected" : ""}>Style v4</option></select></label>`).join("");
}
